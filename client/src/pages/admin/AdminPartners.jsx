...
//     api.get(`/partners?${p}`).then(r => setPartners(r.data.data || [])).finally(() => setLoading(false));
//     // Also fetch unapproved
//     if (filter === 'pending') {
//       // Re-fetch without isApproved filter - admin sees all
//     }
//   }, [filter]);
...
//   const filtered = filter === 'all' ? partners : filter === 'approved' ? partners.filter(p => p.isApproved) : partners.filter(p => !p.isApproved);

//   return (
//     <div>
//       <div className="flex items-center justify-between mb-6">
//         <div>
//           <h1 className="text-2xl font-bold text-gray-900 dark:text-white" style={{ fontFamily: "'Playfair Display', serif" }}>Partners</h1>
//           <p className="text-sm text-gray-500">{partners.filter(p => !p.isApproved).length} pending approval</p>
//         </div>
//       </div>
...
//             className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === v ? 'bg-orange-500 text-white' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700'}`}>
//             {l}
//           </button>
//         ))}
//       </div>
...
//                     <span className={`badge text-xs ${partner.isApproved ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
...
//                 <span className="flex items-center gap-1"><Star size={11} /> {partner.rating?.average?.toFixed(1) || 'New'}</span>
//                 <span className="flex items-center gap-1"><Package size={11} /> {partner.totalOrders} orders</span>
...
//                     className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-sm font-medium bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-100 transition-all">
