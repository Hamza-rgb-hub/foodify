...
exports.createPartner = async (req, res) => {
  try {
    ...
    if (req.files) {
      if (req.files.logo) {
        const result = await uploadToCloudinary(req.files.logo[0].buffer, 'foodify/partners');
        partnerData.logo = result.secure_url;
      }
      if (req.files.coverImage) {
        const result = await uploadToCloudinary(req.files.coverImage[0].buffer, 'foodify/partners');
        partnerData.coverImage = result.secure_url;
      }
    }
